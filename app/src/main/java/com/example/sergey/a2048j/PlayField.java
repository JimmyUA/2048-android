...
public class PlayField implements Parcelable{
    ...
    public PlayField(int size, Score score) {
        Log.d(LOG_TAG, "PlayField(int size)");
        this.setSize(size);
        this.score = score;
    }
    ...
    protected PlayField(Parcel in) {
        size = in.readInt();
        score = in.readParcelable(Score.class.getClassLoader());
    }

    public static final Creator<PlayField> CREATOR = new Creator<PlayField>() {
        @Override
        public PlayField createFromParcel(Parcel in) {
            Log.d(LOG_TAG, "createFromParcel");
            return new PlayField(in);
        }

        @Override
        public PlayField[] newArray(int size) {
    ...
    public void setTilesArray() {
        ...
        addTile(new Tile((size - 1) - (int) (Math.random() * size), (int) (Math.random() * size)));
        addTile(new Tile((size - 1)- (int) (Math.random() * size), (int) (Math.random() * size)));
    }

    /**
     * Method initialising array of tiles with empty tiles
     */
    public void setTilesArrayToNulls() {
        tiles = new Tile[size][size];
    }
